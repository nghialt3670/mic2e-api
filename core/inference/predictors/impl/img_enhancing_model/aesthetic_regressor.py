...
def get_backbone(name, unfreeze_layers_of_vit, img_size):
    ...
    if name == "resnet18":  # 11.7M params
        ...
        model = timm.create_model(
            "vit_base_patch16_224", pretrained=True, img_size=img_size
        )
        ...
        for i in range(
            total_encoder_layers - 1,
            total_encoder_layers - 1 - unfreeze_layers_of_vit,
            -1,
        ):
        ...
        raise ValueError(
            f'Backbone "{name}" not supported, please add backbone in func get_backbone'
        )
...
def get_activation(name):
    if name == "tanh":
        ...
        raise ValueError(
            f'Function "{name}" not supported, please add activation in func get_activation'
        )
...
class AestheticRegressor(nn.Module):
    def __init__(self, **kwargs):
        ...
        unfreeze_layers_of_vit = kwargs.get(
            "unfreeze_layers_of_vit", 1
        ...
        print(
            f"Number of trainable parameters: {sum(p.numel() for p in self.backbone.parameters() if p.requires_grad)}"
        )
